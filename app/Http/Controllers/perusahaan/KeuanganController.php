...
class KeuanganController extends Controller
{
    ...
    public function index(Request $request, string $id)
    {
        ...
        if ($request->has('status')) {
            $projects = InstituteProyeks::where('id_inst', $id)->where('status', $request->status)->get();
        } else {
            $projects = InstituteProyeks::where('id_inst', $id)->get();
        }
        // dd($projects);
    ...
    public function show(Request $request, string $id)
    {
        ...
        if ($type) {
            $tasks = InstitutePengeluaran::where('project_id', $project->id)
                ->where('subject', $type)
                ->orderBy('date', 'asc')
                ->get();
        } else {
